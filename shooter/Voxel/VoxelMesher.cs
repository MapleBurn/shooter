...
public static class VoxelMesher
{
    ...
    public static ArrayMesh GenerateMesh(Shooter.Voxel.VoxelChunk data)
    {
        ...
        {
            for (int y = 0; y < Shooter.Voxel.VoxelChunk.Size; y++)
            {
    ...
    private static bool IsFaceExposed(Faces face, int x, int y, int z, Shooter.Voxel.VoxelChunk data)
    {
        ...
        if (nx < 0 || nx >= Shooter.Voxel.VoxelChunk.Size || 
            ny < 0 || ny >= Shooter.Voxel.VoxelChunk.Size || 
            nz < 0 || nz >= Shooter.Voxel.VoxelChunk.Size)
        {
            return true;
        }
        ...
        List<Vector3> vertices, List<Color> colors, List<Vector3> normals, List<int> indices)
    {
        ...
        foreach (var triangle in triangles)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                vertices.Add((_cubeVertices[vertexIndex] + position) * Shooter.Voxel.VoxelChunk.VoxelSize);
                colors.Add(color);
                normals.Add(normal);
